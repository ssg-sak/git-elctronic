...
| 항목 | 값 |
|---|---:|
| D1 기준시각 | 2026-08-06T20:06:55+09:00 |
| 충전소 D1 행 | 4,210 |
| 공용 기본 후보 | 1,853 |
| 공용·좌표 정상 | 1,848 |
| 공용·확정 가용·좌표 정상 | 1,408 |
| Team5 주차 1km 정적 매칭 | 3,922 |
| Team5 realtime 주차 연결 | 425 |
| UTIC 거리 피처 (D1 기준) | 382 |
| 이용이력 연결 | 200 |
...
- 이 D1은 고정 스냅샷이다. Lightsail pull 또는 UTIC/주차 최신화 뒤에는 **D1 재빌드 전**
  현재 추천값으로 표현하지 않는다.
- D1 UTIC 조인 382개와 현재 조인
  382개가 다를 수 있다. 이는 돌발 자체가 시시각각
  바뀌기 때문이며, 최종 전달 직전 재빌드로 일치시킨다.

```
DA① | integration readiness | 20260806
```
