...
def plot_hourly_heatmap(ats: pd.DataFrame) -> Path:
    ...
    # 일자 늘어도 행이 납작해지지 않게 높이 스케일 (8/9 전후 ~3주 대비)
    n_dates = max(len(pivot.index), 1)
    fig_w = 16.0
    fig_h = max(7.0, min(22.0, 2.2 + n_dates * 0.48))
    fig, ax = plt.subplots(figsize=(fig_w, fig_h), facecolor="#f7f8fa")
    im = ax.imshow(data, aspect="auto", cmap=cmap, vmin=30, vmax=90, interpolation="nearest")
    ax.set_yticks(range(n_dates))
    ax.set_yticklabels(pivot.index, fontsize=11)
    ax.set_xticks(range(24))
    ax.set_xticklabels(list(range(24)), fontsize=11)
    ax.tick_params(axis="both", length=0, pad=4)
    ax.set_title("시간대×일자 패널 가용률 히트맵 (회색=미수집)", loc="left", fontweight="bold", fontsize=14)
    ax.set_xlabel("시 (KST)", fontsize=12)
    cbar = fig.colorbar(im, ax=ax, fraction=0.035, pad=0.02, label="%")
