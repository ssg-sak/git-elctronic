...
## 2026-07-30 — 전처리 5항 (신선도·usage QA·주차1h·D2·linkspeed 보류)

### 한 일

- 전처리 보고서(`docs/전처리/*`)에 2026-07-30 갭·수치를 반영했다.
- D1에 `observation_state` 가산 (UNOBSERVED/STALE 등). 기존 `reliability_grade*` 의미는 유지.
- 이용강도 QA 리포트·메타 `d1_merged=true` · 커버리지 상한(=municipal ~219) 문서화.
- Team5 주차 최근 1h `std/delta/ticks` 파생 → D1 가산 (428소).
- D2 패널 재생성: 1,563 ticks · 4,050,754 rows.
- linkspeed↔충전소 조인은 링크 좌표 부재로 **보류 확정** (`교통소통_데이터_보고.md` §3.5).

### 결정

- D1 스키마는 가산만. grade 재정의·ETA 대체·과거 돌발을 realtime에 섞지 않음.
- usage 4.75%는 조인 버그가 아니라 원천 크기 한계.

### 다음

- 링크 지오메트리 확보 시 linkspeed 소별 조인 스프린트.
- FRESH/NORMAL이 뜨도록 as_of와 스냅 신선도 운영을 맞출지 검토.
